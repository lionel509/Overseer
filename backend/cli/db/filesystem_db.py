...
def get_connection():
    ...
    # Try to add content_summary if not present
    try:
...
def add_file_info(path, type_, size, mtime, tags='', extra='', content_summary=''):
    conn = get_connection()
    c = conn.cursor()
    c.execute('INSERT INTO files (path, type, size, mtime, tags, extra, content_summary) VALUES (?, ?, ?, ?, ?, ?, ?)',
              (path, type_, size, mtime, tags, extra, content_summary))
    conn.commit()
    conn.close()
